...
const ShipmentTable : React.FC = () : React.ReactElement => {
    ...
    return <div className="w-full">
      {isLoading ? <Loading text="Getting shipments..." /> :
            ...
            <Table.Body>
                {shipments.map((item : ShipmentData) => (
                  <Table.Row key={item.id}>
                      ...
                      <Table.Cell><Badge color={getStatusColor(item.status)}>{item.status}</Badge></Table.Cell>
                      <Table.Cell>{item.lastLocation}</Table.Cell>
                      <Table.Cell>{item.date}</Table.Cell>
                      <Table.Cell>
                        <Button className="w-full bg-primary text-white min-w-28" href={`/shipment/${item.id}`}>View</Button>
                      </Table.Cell>
                  </Table.Row>
              ))}
          </Table.Body>
